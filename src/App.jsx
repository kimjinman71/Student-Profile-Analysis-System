...
const DEMO_ANALYSIS_DATA = {
  ...
  subject_specific: [
    ...
    {
      subject_group: "기타 교과군 분석",
      category: "Other",
      strengths: [
        "체육, 미술, 제2외국어 등 다양한 교과에서 성실하게 이수하여 다재다능하고 균형 잡힌 학업 소양을 증명함.",
        "기술 및 코딩 등 실용 교과에서 본인의 주 전공 계열 지식을 연계하여 융합 결과물을 기획 및 구현함.",
        "중국어/일본어 등 제2외국어 학습 시 문화적 차이를 단순 수용하는 것을 넘어 타국 문화의 다양성을 존중하는 개방성을 보임.",
        "진로 및 교양 과목에서 본인의 전공을 관통하는 본인만의 독특한 탐구 쟁점을 발굴하여 학술적으로 설계함."
      ],
      weaknesses: [
        "실기 중심 과목에서 기술적 기교 연마 위주로 기술되어, 해당 분야의 예술 인문학적 배경지식 탐구 깊이가 아쉬움.",
        "코딩/정보 교과에서 기초 알고리즘을 복제하는 수준에 그쳐 전공 도메인 지식을 활용한 실질적 데이터 분석 확장이 부족함.",
        "제2외국어 작문 시 한국어 문장 번역 수준의 어색한 어휘 배치가 빈번하여 원어민의 언어 습관 및 뉘앙스 학습이 요구됨.",
        "비입시 과목에 대한 집중도가 국/수/영/과 주요 과목에 비해 상대적으로 떨어지는 일시적인 참여도 편차가 생기부에 관찰됨.",
        "예술/체육 모둠 과제 시 본인의 역할을 타인에게 미루거나 단순 참여 사실만 기록되어 주도적 이타성이 드러나지 않음.",
        "교양 과목의 과제 완성도가 전반적으로 성실하나, 깊이 있는 질문을 스스로 던지는 학문적 깊이가 상대적으로 아쉬움.",
        "기술/가정 수행평가 시 기획 단계에 비해 결과물의 기능적 정합성 및 실사용 적합도 검증을 위한 최종 성찰이 결여됨.",
...
1. 입력된 학생부 텍스트를 [인적학적사항], [출결사항], [수상경력], [창의적체험활동상황], [교과학습발달상황], [행특] 섹션으로 즉시 분할하고, 각 섹션을 상호 간섭 없이 동시에(Parallel) 파싱하여 핵심 개체(Entity)를 추출하십시오. 
2. **봉사활동실적은 대입 미반영 항목이므로 역량 분석에서 완전히 제외하고 삭제**하십시오.
3. 학생부 특유의 미사여구(예: '우수함', '열심히 참여함', '활동함', '노력함', '관심을 보임', '흥미를 가짐', '경험함', '체험함', '이해함', '맡은 역할을 수행함', '협조함', '접함', '알게됨', '점차 향상됨', '발전 가능성이 있음' 등)는 단순 노이즈로 간주하여 완전히 필터링하여 배제하십시오.
4. 오직 **[동기 -> 구체적 역량 활동 -> 결과 및 변화]** 구조를 가진 의미 있는 인과관계 및 성찰 문맥만 유효 평가 데이터로 인정하여 반영하십시오. 이 과정을 통해 파싱 속도와 분석 정확도를 극대화하십시오.
...
5. 과목순서: 국어교과군, 수학교과군, 영어교과군, 과학교과군, 사회교과군, 기타교과군 순서로 엄격하게 배열하십시오.
...
8. 파일 포맷 처리: 업로드한 학생부 파일이 텍스트 PDF이거나, 스캔본 PDF, 혹은 이미지(스마트폰 촬영본 등) 형태이더라도 전체 학생부 페이지를 정확하고 매우 빠르게 파싱하여 전체 텍스트와 누락된 교과 세특 내용을 누락 없이 완벽히 획득하고 정밀하게 분석을 완료하십시오.
                ...
                <div className="space-y-6">
                  <div className="pb-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-1 bg-slate-100 p-1 border border-slate-200/50 rounded-none self-start sm:self-center">
                      ...
                      <button
                        onClick={() => setActiveSubjectTab("other")}
                        className={`px-3 py-1.5 text-xs font-black transition-all ${
                          activeSubjectTab === "other" ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"
                        }`}
                      >
                        기타교과
                      </button>
                  ...
                  <div className="space-y-6">
                    {analysisResult.subject_specific
                      ?.filter(item => {
                        ...
                        if (activeSubjectTab === "other") {
                          const isMain = ["korean", "math", "english", "science", "social"].includes(cat) ||
                                         group.includes("국어") || group.includes("수학") ||
                                         group.includes("영어") || group.includes("과학") ||
                                         group.includes("사회");
                          return !isMain;
                        }
                        return true;
